...
use quote::{quote};
use syn::{DeriveInput, TypePath, TypeArray, GenericParam, Type, parse_macro_input, token::{Comma}};

use crate::{new_utils::{virually_monomorphize_type, has_engine_generic_param, get_type_params_from_generics}};

pub(crate) fn derive_packable(input: proc_macro::TokenStream, len_ts: &mut TokenStream) -> proc_macro::TokenStream {
    let derived_input = parse_macro_input!(input as DeriveInput);

    let DeriveInput { ident, data, generics, .. } = derived_input.clone();
                    ...
                    for field in named_fields.named.iter() {
                        ...
                        let packing_line = match field.ty {
                            ...
                            Type::Array(ref _arr_ty) => {
                                derive_by_array_type(&field_ident, _arr_ty)
                            },
                        ...
                        let len_line = match field.ty {
                            ...
                            Type::Array(ref _arr_ty) => {
                                create_array_len_extression_recursive(_arr_ty)
                            },
                            _ => abort_call_site!("only array and path types are allowed"),
                        };
    ...
    let _additional_engine_generic = if has_engine_generic_param(&generics.params, &engine_generic_param) {
            quote!{}
        } else {
            quote!{<E:Engine>}
        };

    let inner_const_ident = syn::parse_str::<Ident>(&format!("__{}_circuit_packing_len_inner", ident).to_lowercase()).unwrap();
    let private_mod_ident = syn::parse_str::<Ident>(&format!("__{}_private_consts_packable", ident).to_lowercase()).unwrap();
            ...
            match struct_data.fields {
                syn::Fields::Named(ref named_fields) => {
                    for field in named_fields.named.iter() {
                        ...
                        let packing_line = match field.ty {
                            Type::Path(ref _path_ty) => derive_by_type_path_variable_length(&field_ident, _path_ty),
                            Type::Array(ref _arr_ty) => {

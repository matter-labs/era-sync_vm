...
use quote::{quote};
use syn::{Data, DeriveInput, Fields, Field, TypePath, TypeArray, GenericParam, Generics, Type, parse_macro_input, token::{Comma}, Attribute};

use crate::{new_utils::{has_engine_generic_param, get_type_params_from_generics, get_witness_ident}};
...
pub(crate) fn derive_witnessable(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let serde_remove_bounds = if let Some(serde_remove_bounds) = crate::new_utils::fetch_attr(SERDE_REMOVE_BOUNDS, &attrs) {
                ...
                syn::Fields::Named(ref named_fields) => {
                    for field in named_fields.named.iter() {
                        ...
                        let expanded_init_line = match field.ty {
                            Type::Path(ref _path_ty) => derive_get_witness_by_type(&field_ident, _path_ty),

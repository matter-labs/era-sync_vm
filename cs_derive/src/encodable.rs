...
pub(crate) fn derive_encodable(input: proc_macro::TokenStream, len_expression: TokenStream) -> proc_macro::TokenStream {
    ...
    let inner_const_ident = syn::parse_str::<Ident>(&format!("__{}_circuit_encoding_len_inner", ident).to_lowercase()).unwrap();
    let private_mod_ident = syn::parse_str::<Ident>(&format!("__{}_private_consts_encodable", ident).to_lowercase()).unwrap();

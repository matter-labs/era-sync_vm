...
use crate::{utils::{fetch_attr, get_type_params_from_generics, prepend_engine_param_if_not_exist}, witness::derive_witness_struct};
...
fn derive_fixed_len_encodable_impl(original_struct: &DeriveInput) -> TokenStream{
    ...
    let pack_fn_needs_cs_arg = syn::parse_str::<Expr>(&pack_fn_needs_cs_arg_str).expect("has cs attr as Expr");

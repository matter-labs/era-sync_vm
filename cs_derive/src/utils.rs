...
use syn::{GenericParam, Generics, Type, TypeParam, TypePath, punctuated::Punctuated, token::Comma};


const EQUIVALENT_TYPES: [(&str, &str, Option<&str>, &str, &str); 14] = [
    ("Num<E>", "E::Fr", Some("E: Engine"), "alloc", "get_value"),
    ("UInt256<E>", "BigUint", None, "alloc_from_biguint", "get_value"),
    ("DecommitHash<E>", "BigUint", None, "alloc_from_witness", "create_witness"),
    ...
    ("SmallFixedWidthInteger<E, U160>", "u160", None, "alloc_from_address_like", "get_value_address_like"),
    ("SmallFixedWidthInteger<E, U32>", "u32", None, "from_u32_witness", "get_value_clamped"),
    ("SmallFixedWidthInteger<E, W>", "E::Fr", Some("E: Engine"), "allocate", "get_value"),
];
    ...
    EQUIVALENT_TYPES
        ...
        .map(|(a, b, c, _, _ )| {
            ...
            let type_param = if let Some(ref ty_param) = c {
                Some(GenericParam::Type(syn::parse_str::<TypeParam>(ty_param.clone()).unwrap()))
            }else{
                None
            };
...
pub(crate)  fn get_allocation_fn_name_by_ident(ty: &TypePath) -> Ident{
    EQUIVALENT_TYPES
    .iter()
    .find(|(original_ident, _, _, _, _)| syn::parse_str::<TypePath>(original_ident).unwrap() == *ty)

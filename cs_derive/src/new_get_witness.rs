...
use quote::quote;
use syn::{GenericParam, DeriveInput, Type, TypeArray, TypePath, parse_macro_input, token::Comma};

use crate::{new_utils::{has_engine_generic_param, get_empty_path_field_initialization_of_type, get_equivalent_type, get_type_params_from_generics, get_type_path_of_field, get_base_type_witness_fn_name_by_ident, get_witness_ident}, new_witness::derive_witness_struct};
...
pub(crate) fn derive_get_witness(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
                ...
                syn::Fields::Named(ref named_fields) => {
                    for field in named_fields.named.iter() {
                        ...
                        let expanded_init_line = match field.ty {
                            Type::Path(ref path_ty) => derive_get_witness_by_path(&field_ident, path_ty),
                            Type::Array(ref arr_ty) => {
                                let (expanded_field_init, expaned_array_init)= derive_get_witness_by_array(&field_ident, arr_ty);
    ...
    let additional_engine_generic = if has_engine_generic_param(&generics.params, &engine_generic_param) {
            quote!{}
        } else {
            quote!{<E:Engine>}
        };
...
fn derive_get_witness_by_path(ident: &Ident, ty: &TypePath) -> TokenStream{
    let fn_ident =
        if let Some(base_type_fn_ident) = get_base_type_witness_fn_name_by_ident(ty) {
            base_type_fn_ident
        } else {
            syn::parse_str("get_witness").unwrap()
        };

...
>(
    ...
    let (result, new_of) =
        optimizer.add_addition_relation(cs, src_0, src_1, should_apply, marker)?;

    // set of flag on overflow (mod 2^256), eq flag if addition result == 0 (mod 2^256)
    // and sets gt flag as !of && !eq
    let eq = optimizer.add_zero_check(cs, &result, should_apply, marker)?;
    let gt = Boolean::and(cs, &new_of.not(), &eq.not())?;
    ...
    opcode_partial_application.dst0_value = Some((should_apply, result)); // no divergences

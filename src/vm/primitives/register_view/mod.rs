use super::*;
use crate::traits::*;
use crate::vm::optimization_context::*;
use cs_derive::*;
...
impl<E: Engine> Into<RegisterInputView<E>> for Register<E> {
...
impl<E: Engine> Register<E> {
    ...
    pub fn from_imm(imm: UInt16<E>) -> Self {
        Register {
            inner: [UInt128::from_num_unchecked(imm.inner), UInt128::zero()],
            is_ptr: Boolean::constant(false),
        }
    }
    ...
    pub fn from_uint256<CS: ConstraintSystem<E>>(
        ...
        let high = UInt128::from_num_unchecked(lc.into_num(cs)?);
    ...
    pub fn conditionally_select<CS: ConstraintSystem<E>>(
        ...
        {
            *dst = UInt128::conditionally_select(cs, condition_flag, &a, &b)?;
        }
        if CircuitEq::<E>::eq(&a.is_ptr, &b.is_ptr) {
            result.is_ptr = a.is_ptr;
        } else {
            result.is_ptr = <Boolean as CircuitSelectable<E>>::conditionally_select(
                cs,
                &condition_flag,
                &a.is_ptr,
                &b.is_ptr,
    ...
    pub fn conditionally_reverse<CS: ConstraintSystem<E>>(
        ...
        if CircuitEq::<E>::eq(&a.is_ptr, &b.is_ptr) {
            result_left.is_ptr = a.is_ptr;
            result_right.is_ptr = a.is_ptr;
        } else {
            result_left.is_ptr =
                Boolean::conditionally_select(cs, &condition_flag, &b.is_ptr, &a.is_ptr)?;
            result_right.is_ptr =
                Boolean::conditionally_select(cs, &condition_flag, &a.is_ptr, &b.is_ptr)?;
        }
...
impl<E: Engine> RegisterInputView<E> {
    ...
    pub fn unwrap_as_register(self) -> Register<E> {
        Register {
            inner: self.u128x2_view.unwrap(),
            is_ptr: self.is_ptr,
        }
    }
    ...
    pub fn get_value(&self) -> Option<BigUint> {
        ...
        if self.u128x2_view.is_some() {
            ...
            for chunk in self.u64x4_view.unwrap().iter() {
                if let Some(v) = chunk.get_value() {
                    base += BigUint::from(v) << shift;
                    shift += 64;
                } else {
        ...
        CS: ConstraintSystem<E>,
    {
        ...
        if value.is_constant() {
            ...
            let bytes128_to_160_le_as_integer = u32x8_view[4];

            let mut lc = LinearCombination::zero();
            lc.add_assign_number_with_coeff(&u128x2_view[0].inner, shifts[0]);
            lc.add_assign_number_with_coeff(&bytes128_to_160_le_as_integer.inner, shifts[128]);
            let lowest160 = lc.into_num(cs)?;
            let lowest160 = UInt160::from_num_unchecked(lowest160);
            let decomposed_lowest160 = (
                u64x4_view[0].clone(),
                u64x4_view[1].clone(),
                bytes128_to_160_le_as_integer,
            );
